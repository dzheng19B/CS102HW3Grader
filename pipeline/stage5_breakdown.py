...
def load_results(problem: str, sid: str, lang: str = "python"):
    # Prefer Java results for Java-detected students
    if lang == "java":
        jp = ROOT / problem / "results" / f"{sid}_java.json"
        if jp.exists():
...
def main():
    ...
    for sid in all_ids:
        ...
        p1_lang = p1["detected_language"] if p1 else ""
        p2_lang = p2["detected_language"] if p2 else ""
        p1_score, p1_reason = explain("problem1", sid, p1, load_results("problem1", sid, p1_lang)) if p1 else ("—", "not in P1 manifest")
        p2_score, p2_reason = explain("problem2", sid, p2, load_results("problem2", sid, p2_lang)) if p2 else ("—", "not in P2 manifest")
    ...
    (REPORTS / "score_breakdown.md").write_text("\n".join(out), encoding="utf-8")
    with (REPORTS / "score_breakdown.csv").open("w", newline="", encoding="utf-8") as f:
